...
class RokoviController extends Controller
{
    ...
    public function zakljucaj_ocene(Request $request)
    {
        ...
        $prijaveIds = explode(",", $request->id_prijava);

        $prijave = PrijavaIspita::whereIn('id', $prijaveIds)->get();

        foreach ($prijave as $prijava) {
            $idUpisa = DB::table('ispitne_prijave')->where('id', $prijava->id)->value('id_upis');

            $idStudent = DB::table('upisi')->where('id', $idUpisa)->value('id_student');

            $student = User::where('id', $idStudent)->first();

            if ($student) {
                Log::info('Student email found', ['email' => $student->email]);

                $this->sendEmailNotification($student, $prijava);
            } else {
                Log::error('Student not found for id_upisa', ['id_upisa' => $idUpisa]);
            }
        }

        DB::table('ispitne_prijave')
            ->whereIn('id', $prijaveIds)
            ->update([
                'zakljucana' => 1,
                'potpisao' => Auth::user()->id
            ]);
    ...
    private function sendEmailNotification($student, $prijava)
    {

        $email = new SendGridMail();
        $email->setFrom(env('MAIL_FROM_ADDRESS'), env('MAIL_FROM_NAME'));
        $email->setSubject("Vaša ocena je upisana.");
        $email->addTo($student->email, $student->name);
        $email->addContent(
            "text/html",
            view('emails.ocenaZakljucena', ['student' => $student, 'prijava' => $prijava])->render()
        );
        ...
        try {
            ...
            Log::info('SendGrid response', ['status' => $response->statusCode(), 'headers' => $response->headers(), 'body' => $response->body()]);
        } catch (Exception $e) {
