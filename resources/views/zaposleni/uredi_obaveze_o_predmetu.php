                      ...
                      <div class="form-label">Наставници предавања - група {{$g->broj}}:</div>
                        ...
                        <select id="predavanja{{$g->id}}" name="predavanja{{$g->id}}[]" multiple >
                          @foreach($nastavnici as $nastavnik)
                            ...
                            <option @if($angazovani->where('tip', 'П')->where('broj', $g->broj)->where('id_nastavnika',$nastavnik->id_nastavnika)->first()) selected @endif value={{$nastavnik->id_nastavnika}}>{{$nastavnik->ime." ".$nastavnik->prezime.", ".$nastavnik->naziv_zvanja}}</option>
                            @endif
                          @endforeach
                        </select>
                      </div>
                    ...
                    <div class="form-label">Наставници вежбе - група {{$g->broj}}:</div>
                      ...
                      <select id="vezbe{{$g->id}}" name="vezbe{{$g->id}}[]" multiple >
                        @foreach($nastavnici as $nastavnik)
                          ...
                          <option @if($angazovani->where('tip', 'В')->where('broj', $g->broj)->where('id_nastavnika',$nastavnik->id_nastavnika)->first()) selected @endif value={{$nastavnik->id_nastavnika}}>{{$nastavnik->ime." ".$nastavnik->prezime.", ".$nastavnik->naziv_zvanja}}</option>
                          @endif
                        @endforeach
                      </select>
                    </div>
                    ...
                    <div class="mb-3">
                      <div class="form-label">Наставници ДОН - група {{$g->broj}}:</div>
                        ...
                        <select id="don{{$g->id}}" name="don{{$g->id}}[]" multiple >
                          @foreach($nastavnici as $nastavnik)
                            <option  @if($angazovani->where('tip', 'Л')->where('broj', $g->broj)->where('id_nastavnika',$nastavnik->id_nastavnika)->first()) selected @endif value={{$nastavnik->id_nastavnika}}>{{$nastavnik->ime." ".$nastavnik->prezime.", ".$nastavnik->naziv_zvanja}}</option>
                          @endforeach
                        </select>
                      </div>
